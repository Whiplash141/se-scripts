...
public IEnumerator<Boolean> ListManaging()
{
	...
	if (allBlocks.Count != lastBlockCount)
	{
		...
		} else
		{
			...
			{
				typeCargoDictionary[$"{MoB}{itemType}"] = new List<IMyTerminalBlock>();
			}
		}
...
public IEnumerator<Boolean> FillSpecialBlocks()
{
	...
	foreach (var specialBlock in specialCargoRequestDictionary.Keys)
	{
		...
		for(int i=0; i < requests.Length; i++)
		{
			...
			List<IMyTerminalBlock> sourceList = getDestinationList(id);
...
public IEnumerator<Boolean> AutoAssembly()
{
	...
	foreach (var quotaId in quotaDictionary.Keys)
	{
		...
		if (amount > 0)
		{
			int currentAmount = 0;
			yield return true;
			List<IMyTerminalBlock> sourceBlockList = getDestinationList(quotaId);
			if(sourceBlockList.Count > 0)
			{
				foreach (var cargoBlock in sourceBlockList)
				{
					if (!IsValidCargoBlock(cargoBlock)) continue;
					IMyInventory inv = cargoBlock.GetInventory();
					if (inv.IsItemAt(0))
				...
				if (missingAmount > 0)
				{
					...
					Boolean workingBlueprint = TryGetBlueprint(quotaId, out blueprint);
					yield return true;
					if (workingBlueprint)
					{
						foreach (var assembler in assemblerList)
...
IEnumerable<Boolean> filterBlock(IMyTerminalBlock tb)
{
	...
	if (tb is IMyReactor || tb is IMyCockpit || tb is IMyGasGenerator || tb is IMyGasTank || tb is IMyUserControllableGun || tb is IMyLargeConveyorTurretBase) yield return false;
		...
		} else
		{
			...
			if (ini.TryParse(customData, "ItemFilter", out result))
			{
				...
				foreach (var key in keyList)
				{
					...
					{
						if (ini.Get(key).ToBoolean(false)) typeCargoDictionary[$"{MoB}{keyName}"].Add(tb);
					}
					yield return true;
				}
			}
...
List<IMyTerminalBlock> getDestinationList(MyDefinitionId definitionId)
{
	string itemType = "";
	if(typeCargoCache.TryGetValue(definitionId, out itemType)){
		return typeCargoDictionary[itemType];
	}
	itemType = definitionId.TypeId.ToString();
	if (typeCargoDictionary.ContainsKey(itemType))
	{
		typeCargoCache.Add(definitionId, itemType);
		return typeCargoDictionary[itemType];
	}
...
bool MoveItemToDestination(IMyTerminalBlock sourceBlock, List<IMyInventoryItem> itemList, int i, int invID = 0)
{
	...
	List<IMyTerminalBlock> destionationList = getDestinationList(currentItem.GetDefinitionId());
...
bool TryGetBlueprint(MyDefinitionId itemDef, out MyDefinitionId blueprint)
{
	...
	if (blueprintDictionary.TryGetValue(itemDef, out blueprint))
	{
		return true;
	}
	else
