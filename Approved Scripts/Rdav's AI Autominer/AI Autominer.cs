...
string VERSION = "001"; //Script Version
...
void Main(string argument)
{
	try
	{
		...
		{ Echo("Large Amount Of Thrusters Detected\nProgram Terminated To Prevent Performance Issues\n"+
			...
			"This safety measure can be disabled on line 25"); return; }
		#endregion
...
void MiningLogic(IMyRemoteControl RC, List<Vector3D> DOCK_ROUTE,IMyGyro GYRO,IMyShipConnector CONNECTOR,bool RESET)
{
	...
	//Sets If Full Or Not
	bool IsEmpty = (SHIP_DRILLS[0].GetInventory().CurrentMass < 100); //SHIP_DRILLS[0].GetInventory().MaxVolume - SHIP_DRILLS[0].GetInventory().CurrentVolume > 10 || 
	...
	{ MININGSTATUS.Val = "MINE";}
	if (IsMassAboveThreshold && MININGSTATUS.Val != "FIN" && ISNOTBURIED.Val)
	{  MININGSTATUS.Val = "FULL"; }
...
void BoreMine(IMyRemoteControl RC, Vector3D ROID_START, Vector3D ROID_CENTRE, double ROID_DIAMETER, double SHIPSIZE, bool Reset)
{
	...
	GyroTurn6(X_ADD * 10000000, RotationalSensitvity, GYRO, RC, RollReqt,PrecisionMaxAngularVel);
...
void DeSerializer(string Input)
{
	...
	string[] SplitStrings = SplitInput[0].Split('^');
	...
	string[] SplitInts = SplitInput[1].Split('^');
	...
	string[] SplitVectors = SplitInput[2].Split('^');
	...
	string[] SplitDoubles = SplitInput[3].Split('^');
	...
	string[] SplitBools = SplitInput[4].Split('^');
